...
def button_adresses_action():
    global main_frame
    main_frame.destroy()
    main_frame = tkinter.Frame(top_window)
    main_frame.grid(row=1, column=2)
    adresses.gui_adress_frame(main_frame)
...
def display_window():
    ...
    button_adresses = tkinter.Button(main_menu, text=texte.menu_adresses, command=button_adresses_action)
    ...
    button_adresses.grid(row=3, column=1, sticky=tkinter.W+tkinter.E, pady=2)
